    ...
    return (
        <div>
            <Wrapper>
                ...
                <PageButton onPress={() => handlePagination(1)} active={page === 1} page={1} />

                {page > 3 && <SeparatorButton />}
                {page === totalPages && totalPages > 3 && (
                    <PageButton onPress={() => handlePagination(page - 2)} active={false} page={page - 2} />
                )}
                {page > 2 && <PageButton onPress={() => handlePagination(page - 1)} active={false} page={page - 1} />}
                {page !== 1 && page !== totalPages && (
                    <PageButton onPress={() => handlePagination(page)} active page={page} />
                )}
                {page < totalPages - 1 && (
                    <PageButton onPress={() => handlePagination(page + 1)} active={false} page={page + 1} />
                )}
                {page === 1 && totalPages > 3 && (
                    <PageButton onPress={() => handlePagination(page + 2)} active={false} page={page + 2} />
                )}
